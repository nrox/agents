...
State.prototype.addAction = function (nextState, reward, actionName) {
  ...
  if (this.actions[action.name]) {
    this.actions[action.name].name = action.name;
    this.actions[action.name].nextState = action.nextState;
    if (this.actions[action.name].reward) {
      this.actions[action.name].reward += action.reward;
    } else {
      this.actions[action.name].reward = action.reward;
    }
  } else {
